    ...
    await this.getProfile(userId)
      .then((response) => {
        ...
        if (playerID?.toString() === userId) {
          try {
            let title = this.customParsers.parseConstants(
              BotConstants.PROFILE.TITLE,
              data.name,
              playerID
            );
            ...
            if (data.faction.faction_id !== 0) {
              description = this.customParsers.parseConstants(
                BotConstants.PROFILE.DESCRIPTION,
                data.level,
                data?.faction?.faction_name
              );
              factionDescription = this.customParsers.parseConstants(
                BotConstants.PROFILE.DESCRIPTIONS.FACTION,
              ...
              description = this.customParsers.parseConstants(
                BotConstants.PROFILE.NOFAC_DESCRIPTION,
                data.level
              );
              factionDescription = this.customParsers.parseConstants(
                BotConstants.PROFILE.DESCRIPTIONS.NOFACTION,
                data.name
              );
            }

            let lifeDescription = this.customParsers.parseConstants(
              BotConstants.PROFILE.DESCRIPTIONS.LIFE,
              data.life.current,
              data.life.maximum
            );
            ...
            if (data.job.company_id !== 0) {
              companyDescription = this.customParsers.parseConstants(
                BotConstants.PROFILE.DESCRIPTIONS.COMPANY,
                data.job.position,
                data.job.company_name
              );
            } else {
              companyDescription = this.customParsers.parseConstants(
                BotConstants.PROFILE.DESCRIPTIONS.NOCOMPANY,
                data.name,
                data.job.position
              );
            }

            let marriageDescription;

            if (data.married.spouse_id !== 0) {
              marriageDescription = this.customParsers.parseConstants(
                BotConstants.PROFILE.DESCRIPTIONS.MARRIAGE,
              ...
              marriageDescription =
                BotConstants.PROFILE.DESCRIPTIONS.NOMARRIAGE;
            }

            let links = this.customParsers.parseConstants(
              BotConstants.PROFILE.DESCRIPTIONS.LINKS,
            ...
            profileEmbed
              ...
              .addField(BotConstants.PROFILE.TITLES.FACTION, factionDescription)
              .addFields(
                {
                ...
                },
                {
                  ...
                  value:
                    Companies[
                      `C${data.job.company_type}` as keyof typeof Companies
                    ],
                  inline: true,
                }
              )
              .addField(
                BotConstants.PROFILE.TITLES.MARRIAGE,
                marriageDescription
              )
              ...
              .setColor(
                States[`${data.last_action.status}` as keyof typeof States]
              );
